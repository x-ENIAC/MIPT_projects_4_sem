...
class Rectangle {
  public:
	...
	void draw_contour(SDL_Renderer* render, Colour colour_contour) {
		Vector left_side(get_left_down_corner(), get_left_up_corner());
		Vector up_side(get_left_up_corner(), get_right_up_corner());
		Vector right_side(get_right_up_corner(), get_right_down_corner());

		left_side.draw(render, colour_contour);
		up_side.draw(render, colour_contour);
		right_side.draw(render, colour_contour);
	}
	...
	Point get_right_down_corner() const {
		return Point(center.x + width / 2, center.y + height / 2);
	}	
